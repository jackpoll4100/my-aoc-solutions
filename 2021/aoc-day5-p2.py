...
for z in list1:
    if int(z[0][0]) == int(z[1][0]):
        ...
        if int(z[0][1]) < int(z[1][1]):
            xIndex = int(z[0][0])-1
            yIndex = int(z[0][1])-1
            while xIndex < int(z[1][0]):
                matrix[xIndex][yIndex] += 1
                if matrix[xIndex][yIndex] > 1 and [xIndex, yIndex] not in result:
                    result.append([xIndex, yIndex])
                yIndex += 1
                xIndex += 1
        if int(z[0][1]) > int(z[1][1]):
            xIndex = int(z[0][0])-1
